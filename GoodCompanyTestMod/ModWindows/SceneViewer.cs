...
{
	public class SceneViewer : BaseModWindow
	{
		...
		public SceneViewer(bool active = true)
		{
			Activated = active;
		}
		...
		{
			GUI.Label(new Rect(20, 20, 300, 20),
				$"{_inspectedObjComponents.Count} components on {_inspectedObject.name}");

			const float Y_OFFSET = 20.0f;

			inspectComponentsScrollPos = GUI.BeginScrollView(new Rect(10, 70, 280, 450), inspectComponentsScrollPos,
				new Rect(0, 0, 260, _inspectedObjComponents.Count * Y_OFFSET));
